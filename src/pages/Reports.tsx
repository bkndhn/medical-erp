...
export default function Reports() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setDevices((dev as any) || []); setLoading(false);
    });
  }, [tenantId]);
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>{expiryItems.map((item, i) => (
                    <tr key={i} className={`border-b border-border/30 ${item.expired ? "bg-destructive/5" : item.daysLeft <= 30 ? "bg-accent/5" : ""}`}>
                      ...
                      <td className="py-2 text-right"><span className={`font-semibold ${item.expired ? "text-destructive" : item.daysLeft <= 30 ? "text-accent" : "text-success"}`}>{item.expired ? "EXPIRED" : `${item.daysLeft}d`}</span></td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
