...
import {
  ...
  Settings, TrendingUp, Truck, CreditCard, Building2,
  Monitor, ChevronLeft, ChevronRight, Zap, HelpCircle,
  BarChart3, Wallet, MessageSquare
} from "lucide-react";
  ...
  const renderNavItem = (item: typeof mainNav[0] & { highlight?: boolean }) => (
    <li key={item.url}>
      ...
      >
        ...
        {!collapsed && item.highlight && !isActive(item.url) && (
          <Zap className="h-3 w-3 ml-auto text-accent" />
        )}
      </button>
    </li>
  );
      ...
      <div className="flex items-center gap-3 px-4 h-16 border-b border-sidebar-border shrink-0">
        ...
        {!collapsed && (
          <div className="overflow-hidden">
            ...
            <p className="text-[10px] text-muted-foreground">Multi-Industry SaaS</p>
          </div>
        )}
      </div>
